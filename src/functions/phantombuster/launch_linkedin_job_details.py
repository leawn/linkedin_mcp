...
class LaunchJobDetailsInput(BaseModel):
    """Input parameters for launching a LinkedIn job details export."""
...
def raise_exception(message: str) -> None:
    log.error("launch_linkedin_job_details_phantombuster function failed", error=message)
    raise NonRetryableError(message)


@function.defn()
async def launch_linkedin_job_details_phantombuster(function_input: LaunchJobDetailsInput | None = None) -> dict[str, Any]:
    """Launches the LinkedIn job details Phantom and returns the container ID immediately.
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            log.info("Launching LinkedIn job details export with saved default input")
            ...
            log.info(f"LinkedIn job details export launched. Container ID: {container_id}")
            return {"status": "launched", "containerId": container_id}

    except Exception as e:
        error_message = f"launch_linkedin_job_details_phantombuster failed: {e}"
