...
class GetJobDetailsInput(BaseModel):
    ...
    job_urls: list[str] = Field(
        ...
        description="The LinkedIn job post URLs to scrape.",
        min_length=1,
...
async def get_linkedin_job_details_phantombuster(function_input: GetJobDetailsInput) -> dict[str, Any]:
    try:
        ...
        argument = {
            ...
            "jobsToScrape": "\n".join(function_input.job_urls),
            "homerun": True,
        }

        async with httpx.AsyncClient() as client:
            log.info(f"Initiating LinkedIn job details scrape for {len(function_input.job_urls)} jobs")
